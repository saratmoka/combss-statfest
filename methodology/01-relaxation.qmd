...
The homotopy in $\delta$ — gradually increasing $\delta$ during optimisation — uses the small-$\delta$ smooth landscape for navigation and the large-$\delta$ peaked landscape to push $\boldsymbol{t}$ to a vertex (= a discrete subset). See [Homotopy](03-homotopy.qmd).
...
Together they motivate the **homotopy in $\delta$**: start small for a smooth landscape, end large to land on a discrete solution (see [Homotopy](03-homotopy.qmd)). The outer optimisation along the way needs the gradient $\nabla f_\delta$ — derived next on the [Frank-Wolfe](02-frank-wolfe.qmd) page.
